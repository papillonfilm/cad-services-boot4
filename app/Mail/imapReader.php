...
class imapReader
{
    ...
    public function connect()
    {
        if ($this->canConnect())
        {
            ...
            $this->box = @imap_open("{{$this->host}:{$this->port}/pop/ssl/novalidate-cert}INBOX", $this->user,
                $this->pass);

//            $this->box = @imap_open("{{$this->host}:{$this->port}/imap/ssl}Inbox", $this->user,
//                $this->pass);
    ...
    private function _fetchHeader($mbox, $id)
    {
        ...
        $mail->fromaddress = $this->_fix(isset($header->from[0]->mailbox) ? ($header->from[0]->mailbox) : (''));
        $mail->fromhost = $this->_fix(isset($header->from[0]->host) ? ($header->from[0]->host) : (''));
        $mail->ccaddress = $this->_fix(isset($header->cc[0]->mailbox) ? ($header->cc[0]->mailbox) : (''));
        $mail->cchost = $this->_fix(isset($header->cc[0]->host) ? ($header->cc[0]->host) : (''));
        ...
        else
        {
            ...
            foreach ($parts as $part)
            {
                ...
                if ((isset($part->data->disposition))
                    && ((strcmp('attachment', strtolower($part->data->disposition)) == 0)
                        || (strcmp('inline', strtolower($part->data->disposition)) == 0)))
                {
                    ...
                    if (isset($part->data->dparameters))
                    {
                        ...
                        $content->attachment['is_attachment'] = true;
                    }
                    ...
                    //added for downloading attachments
                    if($content->attachment['is_attachment'])
                    {
                        $content->attachment['attachment'] = imap_fetchbody($this->box, $mail->id, $i +1);

                        /* 3 = BASE64 encoding */
                        if($part->data->encoding == 3)
                        {
                            $content->attachment['attachment'] = base64_decode($content->attachment['attachment']);
                        }
                        /* 4 = QUOTED-PRINTABLE encoding */
                        elseif($part->data->encoding == 4)
                        {
                            $content->attachment['attachment'] = quoted_printable_decode($content->attachment['attachment']);
                        }
                    }
