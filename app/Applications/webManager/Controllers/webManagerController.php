...
//            echo $route->uri." - ".$route->getName()." - ".$route->getPrefix()." - ".$route->getActionMethod()."<br />";
//        }
		...
		foreach($routes as $route){
            ...
            //echo $route->uri."<br />";
            //print_r($route->action['middleware']);
            ...
            $systemRoutes[$counter]['applicationName'] = $route->getName();
		...
		foreach($systemRoutes as $key=>$route){
				 ...
				 if( $route['sync'] == 0 and $route['add']==1){
					...
					$pieces = explode("/",$route['url']);
					$applicationName = $pieces[1];
